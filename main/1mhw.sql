...
INSERT INTO WORKER (FIRST_NAME, LAST_NAME, SALARY, JOINING_DATE, DEPARTMENT ) 
	VALUES ('ANNNORA','MCCARTNEY', 25000 ,CURRENT_TIMESTAMP, 'MARKETING');
...
-- HAVE 20 ENTRIES
INSERT INTO TITLE(TITLE_ID, WORKER_TITLE, AFFECTED_FROM) 
  VALUES(1, 'MANAGER', CURRENT_TIMESTAMP);

-- STEP 3. COMPLETE 50 TASKS

-- 1 
  SELECT FIRST_NAME AS WORKER_NAMES 
  FROM WORKER;
...
-- 3
  ...
  FROM WORKER; --  ИСКЛЮЧАЕТ СТРОКИ СОВПАДАЮЩИЕ ПО ВСЕМ УКАЗАННЫМ ВЫРАЖЕНИЯМ
...
-- 6
  ...
  FROM WORKER; -- УДАЛЕНИЯ ПРОБЕЛОВ С ПРАВОЙ СТОРОНЫ

-- 7
  SELECT LTRIM(FIRST_NAME) 
  FROM WORKER; -- УДАЛЕНИЯ ПРОБЕЛОВ С ЛЕВОЙ СТОРОНЫ
...
-- 13
  SELECT * FROM WORKER 
  WHERE FIRST_NAME IN ('CECIL', 'DAMIEN');

-- 14
  SELECT * FROM WORKER 
  WHERE FIRST_NAME NOT IN ('CECIL', 'DAMIEN');
...
-- 18
  SELECT * FROM WORKER 
  WHERE FIRST_NAME LIKE '_______H';
...
-- 20
  SELECT * FROM WORKER 
  WHERE TO_CHAR(JOINING_DATE, 'MON') = 'MAR'
...
-- 23
  SELECT DEPARTMENT, COUNT(WORKER_ID) NUM_OF_WORKERS 
  FROM WORKER 
  GROUP BY DEPARTMENT 
...
-- 24
  ...
  FROM WORKER W 
  INNER JOIN TITLE T ON W.WORKER_ID = T.TITLE_ID AND T.WORKER_TITLE IN ('MANAGER')
...
-- 26
  SELECT * FROM WORKER 
  WHERE MOD(WORKER_ID, 2) <> 0; -- НЕ ЧЕТНЫЕ СТРОКИ

-- 27
  SELECT * FROM WORKER 
  WHERE MOD (WORKER_ID, 2) = 0; -- ЧЕТНЫЕ СТРОКИ

-- 28
  SELECT * INTO WORKERCLONE 
  FROM WORKER; --CLONE TABLE
...
-- 44
  SELECT * FROM WORKER 
  WHERE WORKER_ID <=5 UNION SELECT * FROM 
